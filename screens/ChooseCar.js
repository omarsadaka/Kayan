...
import {
  ...
  ActivityIndicator, AsyncStorage, Picker, BackHandler, RefreshControl, Dimensions, SafeAreaView
} from 'react-native';
...
class ChooseCar extends Component {
  ...
  _retrieveData = async () => {
    ...
    const Arr = await AsyncStorage.getItem('Help');
        if (Arr) {
          const helpArr = JSON.parse(Arr);
  ...
  getData = () => {
    NetInfo.fetch().then(state => {
      if (state.isConnected) {
        fetch('http://134.209.178.237/api/user/getAllCars')
          .then((response) => response.json())
          .then((responseJson) => {
            ...
            // if (Data.length > 0) {
              ...
              for (let index = 0; index < Data.length; index++) {
                ...
                // else if(booking_year==year && booking_month>=month){
                //   if(Data[index].status!=4) {

                //     carInfo.push(obj)
                //   }   
                // }else if(booking_year>=year){
                //   if(Data[index].status!=4) {
               ...
               if(carInfo.length === 0){
                if (this.state.lang.indexOf('ar') != -1) {
                this.setState({ flag_lang: 1 });
                alert(' لا يوجد سيارات  للأيجار الأن');
              } else {
                this.setState({ flag_lang: 1 });
                alert(' No cars for rent now')
              }
               }else{
            ...
            // } else {
            //   // this.setState({flag_block:1})
            //   this.setState({ refreshing: false })
            //   if (this.state.lang.indexOf('ar') != -1) {
            //     this.setState({ flag_lang: 1 });
            //     alert(' لا يوجد سيارات  لهذة الفئه');
            //   } else {
            //     this.setState({ flag_lang: 1 });
            //     alert(' No cars for this category')
            //   }
            // }
          })
          .catch((error) => {
      ...
      <View style={{ width: width * 0.6, padding: 2, flexDirection: 'row',}}>
        ...
        <TouchableOpacity
          ...
          onPress={async() => {
            ...
            const obj2 ={
              carID:item.id,
              startDate:this.state.helpArr.startDate,
              endDate:this.state.helpArr.endDate,
              startTimeH:this.state.helpArr.startTimeH,
              startTimeM:this.state.helpArr.startTimeM,
              endTimeH:this.state.helpArr.endTimeH,
              endTimeM:this.state.helpArr.endTimeM,
              receving:this.state.helpArr.receving,
              delivering:this.state.helpArr.delivering,
              insurance:item.insurance
            }
            await AsyncStorage.setItem('Step1',JSON.stringify(obj2));

            this.props.navigation.navigate('Additions', { car_id: item.id,track:2})
          }}
        >
      ...
      <View style={{ width: '100%', height: '8%', alignItems: 'center', justifyContent: 'center' }}>
          <View style={[this.state.lang ==='ar'?styles.row_res:styles.row,{width: width, height: '100%', alignItems: 'center', justifyContent: 'center',backgroundColor: '#C8972C', }]}>
            ...
            <Text style={{ textAlign: 'center',  width:'80%', fontSize: 20,fontFamily:'segoe', color: "#fff",   }}>
              {this.state.lang.indexOf('ar') != -1 ? 'أختر السيارة' : 'Choose car'}
            </Text>
            {this.state.lang==='ar'?
    ...
    return (
      <SafeAreaView style={{ flex: 1, backgroundColor:'#FFF', alignItems: 'center', }}>
        ...
        {this.state.flag_lang == 0 ?
          ...
          :
          <View style={{ width: '97%', flexDirection: 'column', alignItems: 'center',elevation:5,shadowOpacity:0.3,backgroundColor:'#fff',
          marginTop:5,borderRadius:5,height:'90%' }}>
           ...
           <View style={{width:width,alignItems:'center',justifyContent:'center',marginTop:20,marginBottom:10}}>
            ...
            <View style={[this.state.lang ==='ar'?styles.row:styles.row_res,{width:'96%',alignItems:'center',justifyContent:'center'}]}>
